...
if (Meteor.isServer) {
    Meteor.startup(function () {
        ...
        if (!Consults.findOne()) {
            // create the example consult that installs with application.   Instant gratification.
            var consult =
                {
                    shortName: "First Test Consult",
                    tweetHeader: "Pharmacist On-Call:  ",
                    phoneMessage: "Thanks for calling me back. This is the sample kon sult that comes with the application.  If this were an actual kon sult, I would probably ask something like:  What do you think we should do with this patient?  Say something clever after the beep and then just press any key when you are done.",
        ...
        if (!Credentials.findOne()) {
            // Placeholder.   There is only ever one entry in this collection.
            var twilio =
            {
                accountsid: "null",
                authtoken: "null",
        ...
        // Keep Alive by checking Twilio every 15 minutes
        Meteor.setInterval(function () {
            var ping = Meteor.call("confirmTwilioCredentials");
            console.log(ping);
        }, 900000);
