...
Router.map(function () {
    ...
    //  clicking about takes the user to the git hub site
    ...
    // import a spreadsheet containng students
    ...
    this.route('/sendConfirmation', {
        ...
        action: function () {
            ...
            Meteor.call('shortenURL', RecordingURLMP3, function (error, result) {
                if (!error && result.length > 0 && consultdata.shortURL) {  // if we have a shortened URL and want to use it send it
                    shortRecordingURLMP3 = result;
                    Meteor.call('sendSMS', Caller, Called, "You can review your consult response at " + shortRecordingURLMP3, function (error, result) {  // to, from, message
                        if (!error) {
                            // console.log(result);
                        } else {
                            // return error
                ...
                } else { // if can't get shortened URL (or don't want short URL), send original
                    Meteor.call('sendSMS', Caller, Called, "Review consult response:  " + RecordingURLMP3, function (error, result) {  // to, from, message
                        if (!error) {
                            // console.log(result);
                        } else {
                            // return error
            ...
            try {
                // try to send email confirmation, but don't cause a server error if it fails to connect
                // also, this does not use shortened URLs.  It hides the long URL in a hyperlink, instead
                    var student = Students.findOne({phone: Caller});
                    if (student) {
                        var to = student.email;
                        var subject = "On-Call Assignment Response";
                        var html = 'You can review your consult response at by clicking <a href="' + RecordingURLMP3 + '">here</a>.';
    ...
    this.route('exportResponsesCSV', {
        ...
        action: function () {
            ...
            var csv = '"created","recordingURL","transcript","callerID","phoneRegistrant","callSid","start_time","studentLastName",' +
                '"studentFirstName","studentID","studentUsername",' +
                '"studentEmail","studentGradYear","studentPhone","consultName","phoneMessage","hangupMessage","tweetHeader",' +
                '"consultURL","keyURL","MaxSeconds"\n';
            Responses.find({consult_id: this.params._id}).forEach(function (response) {
                ...
                csv += '\"' + response.callSid + '\",';
                csv += '\"' + response.start_time + '\",';
    ...
    // displays the consult webpage to the student
    this.route('oncall', {
        path: '/oncall/:_id',
        data: function () {
            if (Session.get('consultCounted') != 'true') {
                Meteor.call('updateConsultPageCount', this.params._id, function (error, result) {
                    Session.set('consultCounted', 'true');
                });
            }
        ...
        ,
        layoutTemplate: 'layoutStudentPages',
        waitOn: function () {
            return Meteor.subscribe('consultpages');
        }
    });

    //displays the key webpage to the student
    this.route('key', {
        path: '/key/:_id',
        data: function () {
        ...
        layoutTemplate: 'layoutStudentPages',
        waitOn: function () {
            return Meteor.subscribe('keypages');
        }
    });
